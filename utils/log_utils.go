...
func InitLogrus(logPath string, logFileName string, maxAge time.Duration, RotationSize int64, rotationTime time.Duration, RotationCount uint) {
	...
	writer, err := rotatelogs.New(
		...
		rotatelogs.WithRotationSize(RotationSize), //日志切割大小
		//rotatelogs.WithRotationCount(RotationCount),//日志保留条数与文件保持最大时间 maxAge 不能共存
	)
	if err != nil {
