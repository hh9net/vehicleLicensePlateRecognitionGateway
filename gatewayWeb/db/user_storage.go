...
import (
	...
	"vehicleLicensePlateRecognitionGateway/gatewayWeb"
	"vehicleLicensePlateRecognitionGateway/utils"
)

//用户注册 查询用户信息是否以及存在
func QueryUserLoginmsg(username string) (error, *gatewayWeb.BSysYongh) {
	db := utils.GormClient.Client
	user := new(gatewayWeb.BSysYongh)
...
//用户注册 插入数据库
func UserInsert(data *gatewayWeb.BSysYongh) error {
	db := utils.GormClient.Client
